...
from utils.db.server_currency_db import fetch_all_fry_points
...
async def make_fry_point_leaderboard(bot: discord.Client):
    ...
    if not fry_points_data:
        return "No fry points data available."
    ...
    embed = discord.Embed(title="🍟 Fry Point Leaderboard 🍟", color=discord.Color.gold())
...
async def fry_point_reset(bot: discord.Client):
    ...
    if not top_fry_points:
        pretty_log("info", "No fry points data found to reset.", label="FRY POINT RESET")
        return

    guild = bot.get_guild(CELESTIAL_SERVER_ID)
    if not guild:
        pretty_log("error", "Guild not found. Cannot announce fry point reset.", label="FRY POINT RESET")
        return

    # Channel
    news_channel = bot.get_channel(CELESTIAL_TEXT_CHANNELS.clan_annoucement)
    if not news_channel:
        pretty_log("error", "News channel not found. Cannot announce fry point reset.", label="FRY POINT RESET")
        return
...
The sacred fryer oil has settled… the golden potatoes have been counted… and the Fry Elders have revealed this week’s most devoted worshippers. 🙏✨"""
    leaderboard_embed = await make_fry_point_leaderboard(bot)

    # Prepare the announcement message
    message = None
    if len(top_fry_points) == 1:
        user_id = top_fry_points[0]
        points = top_fry_points[0]["fry_points"]
        user = bot.get_user(user_id)

        if user:
            message = f"""👑🍟 ALL HAIL THE SUPREME FRY DISCIPLE 🍟👑

After countless prayers, dangerous levels of grease inhalation, and unwavering devotion to the sacred fryer… {user.mention} has officially claimed 1ST PLACE at the Fry Praying Shrine with **__{points}__** {FRY_POINT_EMOJI}. 🙏✨
...
Everyone congratulate {user.mention} for becoming this week’s High Priest of the Holy Fry Shrine 🍟✨"""
    ...
    try:
        if leaderboard_embed:
            await news_channel.send(content=leaderboard_message, embed=leaderboard_embed)
        if message:
            await news_channel.send(content=message)
        elif message is None:
            # Ask clan staff to decide how to handle ties
            staff_room_channel = bot.get_channel(CELESTIAL_TEXT_CHANNELS.moderator_only)
            if staff_room_channel:
                await staff_room_channel.send(content=f"Hey clan staff, we have a tie for the most fry points this month. Please decide how to handle the tie and announce it in {news_channel.mention}.")
        # Reset fry points after announcing the leaderboard
        await reset_all_fry_points_only(bot)

    except discord.HTTPException as e:
        pretty_log("error", f"Failed to send fry point reset announcement: {e}", label="FRY POINT RESET")
        return
