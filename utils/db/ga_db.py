...
CREATE TABLE giveaways (
    giveaway_id SERIAL,
    message_id BIGINT UNIQUE,  -- make message_id unique for ON CONFLICT
    channel_id BIGINT,
    host_id BIGINT,
    host_name TEXT,
    ends_at BIGINT,
...
CREATE TABLE giveaway_entries (
    ...
    entry_count INT,
    joined_at BIGINT,
    PRIMARY KEY (giveaway_id, message_id, user_id),
    CONSTRAINT fk_giveaway
        FOREIGN KEY (giveaway_id, message_id)
        REFERENCES giveaways (giveaway_id, message_id)
        ON DELETE CASCADE
);
"""

def determine_giveaway_type_via_channel_id(channel_id: int) -> str:
    """Returns the value clan"""
    return "clan"
